import ContratoPjAnexo from '#models/contrato_pj_anexo'
import type { HttpContext } from '@adonisjs/core/http'
import app from '@adonisjs/core/services/app'
import fs from 'node:fs'

export default class ContratoPjAnexosController {
  async store({ request, params, response }: HttpContext) {
    try {
      const contratoId = params.contrato_pj_id
      ...
      for (const file of files) {
        ...
        await file.move(app.publicPath('uploads/contractsPJ'), {
          name: fileName,
        })

        const anexo = await ContratoPjAnexo.create({
          contrato_pj_id: contratoId,
          file_name: file.clientName,
          file_path: `/uploads/contractsPJ/${fileName}`,
  ...
  async index({ params, response }: HttpContext) {
    const contratoId = params.contrato_pj_id
    const anexos = await ContratoPjAnexo.query().where('contrato_pj_id', contratoId)
    let prefixUrl = ''
    ...
    if (!contratoId) {
      return response.status(400).send('O ID do contrato é necessário')
    }
  ...
  async show({ params, response }: HttpContext) {
    try {
      const anexo = await ContratoPjAnexo.findOrFail(params.id)
